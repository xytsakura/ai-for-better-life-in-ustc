...
- 使用运行时 secret 文件的 Featured Agent 必须在每次 token 兑换前重新读取文件；bootstrap 轮换密钥后不能依赖人工重启进程。文件缺失或为空时必须拒绝兑换，不得回退内存中的旧文件密钥。
...
- 每次影响交付的功能、修复、重构、测试或重要文档提交，都必须同步更新根目录 `CHANGELOG.md`；涉及具体应用时同时更新该应用的 `CHANGELOG.md`。推送前将日志完整性作为固定检查项。
- 纯格式、生成文件和只维护更新日志的提交不要求递归记录自身，避免产生无限的日志维护提交。
...
- 当前工作目录已验证 ASCII junction：`D:\codex_links\ai_for_ustc_107` -> `D:\HuaweiMoveData\Users\14866\Desktop\107`；Codex Desktop 返回本地文档链接时优先使用该别名。
