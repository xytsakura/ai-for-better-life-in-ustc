...
平台原设计聚焦白名单独立 Agent、两个内部 Demo 和统一 Gateway。团队进一步明确了更长期的产品形态：平台不仅接入外部校园 Agent，还应提供一套共享校园 Agent Harness，让每个用户可以选择不同模型、绑定公共或私人知识、维护自己的个人 Agent，并通过缓存复用大家已经完成的公开调研。
...
ADR-0002 的纵向闭环原则继续有效，但新增一个受严格约束的横向证明：同一模板在平台模型和本地 Runner 模型上运行，业务流程代码不变，且可以复用公共证据缓存。

为控制范围：

- 新增 Harness/Profile/Adapter/Runner 的最小契约与演示；
- 平台托管只使用团队受控测试凭据；
- 只实现一个 OpenAI-compatible adapter 和两个白名单 provider 配置；
- 只实现一个 CLI Runner、一个设备、一个并发任务；
- 只演示 L3 证据、L4 公共答案和 L5 私人生成三类关键缓存；
- 自动路由、任意 provider、多设备、真实托管 BYOK、市场和计费全部后置。

若到 2026-07-27 仍无法完成 Profile 解析、两种执行模式和缓存/安全最小 spike，则个人 Agent 能力降级为“规格 + 录制的接口级验证”，不阻塞 Gateway、课程评价和 RAG 三条原核心链路。
