...
- 导出演示日志时必须脱敏或使用模拟用户。
- `AgentProfile` 只保存知识空间引用；Profile 必须属于当前用户，且只能绑定该用户有权访问的公共、课程共享和私人空间。
- Profile 删除时撤销其知识空间授权与私人生成缓存，但不自动删除用户文件；文件、记忆和 Profile 分别提供删除入口。
...
- 检索出的 `private` chunk、个人记忆和 `local_authenticated` 证据默认只能交给本地 Runner；逐次确认远端 provider 和数据范围后才允许例外，且比赛 MVP 不远端发送登录限定内容。
- Runner 离线或当前模型失败时不得静默切到云模型或平台 key。

## 14. 缓存

本 Agent 对齐 Harness 的五层缓存：

- L1 来源快照：公开/获授权的文档版本和来源元数据。
- L2 解析与索引：相同文件指纹复用解析结果，相同分块指纹与 embedding 模型版本复用向量。
- L3 证据与检索：规范化问题到 `EvidenceBundle`；公共或相同共享 scope 可复用，私人查询按用户隔离。
- L4 公共标准答案：只使用公共证据，经引用和安全校验后发布，记录 generator、model、prompt 和 evidence 版本。
- L5 私人生成：结合个人资料、记忆或用户模型的回答，只存在用户私有空间或本地加密缓存。

不同生成模型可以共享 L1-L3，不能跨用户共享 L5。L4 命中不消耗用户模型额度；“用我的模型重新生成”复用 L3，并把 usage 与结果写入本人 L5。用户 BYOK 回答默认不进入 L4。

缓存键必须包含资料与权限版本；L4 还包含模板、提示、generator provider/model 与证据版本；L5 还包含 `subject_id`、`agent_profile_id`、provider、model、私人资料与记忆版本。任何资料删除、失效、授权变化或新版本发布后，相关缓存立即失效。`private` 与 `local_authenticated` 不得影响 L1-L4。
...
5. 生成回答时按来源分组引用，避免混淆。
6. Profile 选择的模型只影响生成步骤，不改变检索 SQL 的权限过滤；模型 adapter 不能自行扩大空间范围。
...
- 权限过滤在 ModelProfile 解析和模型 egress 之前完成；执行快照记录 `subject_id`、data scope、provider、model、runtime 和 policy 版本，但审计日志不保存私人 chunk、完整问题或记忆正文。
...
- 通过统一 AgentTemplate 在平台/团队测试模型与本地 Runner 模型上完成同一复习问题，检索和引用流程代码不变。
