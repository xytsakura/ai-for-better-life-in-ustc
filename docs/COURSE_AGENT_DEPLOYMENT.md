...
当前可运行产品是作为 Campus Agent Hub 首个 Featured Agent 的瀚海行 `v0.9.0`。仓库已包含 Hub、瀚海行和独立校园助手 Demo 的三服务闭环；成功部署后可以：

- 在管理员 `demo-a`、投稿作者 `demo-b` 和普通订阅者 `demo-c` 三个演示身份之间切换；
- 访问彼此隔离的个人知识空间，以及两人都可访问的邀请制共享空间；
- 浏览仓库预置的 25 份唯一数学分析 B1 资料及其解析状态；
- 向可写知识空间上传不超过 50 MiB 的 PDF，进行页级解析、分块与 SQLite FTS5 全文索引；
- 把个人资料以不可变快照投稿到知识广场，经管理员审核后供普通用户免费订阅和只读使用；
- 对文档执行重复检测、重新解析与删除，删除后关联索引立即失效；
- 在主问答和 GPT-5.6 独立解释分支中实时接收 Responses-compatible 模型的流式正文；
...
当前已实现比赛 Demo 所需的 Agent Portal、Registry、审核、Gateway、Featured 工作台和多模型配置闭环；生产级统一登录、公开网络部署和第三方生态运营仍属于后续阶段。
...
该脚本验证首页、前端脚本、样式、KaTeX、数据库、FTS5、三个演示身份、个人/共享空间、模型配置状态和 25 份唯一课程资料，但不会向模型发送问题。如果暂时没有 API Key，去掉 `--require-llm` 即可验证离线部分。
...
13. 在 `demo-b` 的个人空间上传测试 PDF，点击“投稿到广场”并提交审核；确认待审版本不会出现在普通广场列表。
14. 切换到 `demo-a` 的“知识广场 → 待审核”，审阅并批准该版本。
15. 切换到 `demo-c`，订阅刚发布的知识库并进入独立只读空间；取消订阅后确认空间入口与 RAG 访问立即失效。
...
如果在 Uvicorn 前增加 Nginx 或其他反向代理，必须为 `/api/query/stream` 和 `/api/branch-query/stream` 关闭响应缓冲与缓存，并保持足够长的读取超时。否则后端已经逐步发送 SSE，代理仍可能等到完整回答后一次性交给浏览器。
...
先确认服务商的 `/responses` 端点支持 `stream=true` 和 Responses SSE 事件，而不只是一次性 JSON。若应用前方存在 Nginx、CDN 或其他代理，再检查其是否缓冲 `text/event-stream` 响应。应用不会把完整答案在浏览器端切片伪装成流式输出。
...
- `demo-a` 的个人资料是否对 `demo-b`、`demo-c` 不可见；
- 待审资料是否不可被普通用户访问，订阅取消、知识库暂停和版本切换后旧文档 ID 是否即时失效；
