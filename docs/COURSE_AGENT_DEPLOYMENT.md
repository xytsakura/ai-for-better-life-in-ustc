...
`COURSE_AGENT_LLM_BASE_URL` 应填写基础地址，例如服务商要求的地址通常以 `/v1` 结尾。应用会自行追加 `/responses`，不要把 `/responses` 重复写入基础地址。模型名必须以实际服务商提供的模型 ID 为准；如果队友使用的模型不是 `gpt-5.6-sol`，只需修改这一项。
...
4. 保持“直接问答”，询问“一致连续和连续有什么区别？”。回答不应出现课程资料引用。
5. 切换“使用课程资料”，确认页面展示可以逐份勾选的资料列表。
6. 点击“日常学习”，应优先选中讲义、笔记和提纲；点击“备考刷题”，应优先选中真题、答案和解析。
7. 选择一份或多份资料后提问，确认回答仅引用本轮选中的文件，并显示文件名和页码。
8. 在回答中检查普通 Markdown 列表、行内公式和块级公式是否正常渲染，不应看到未渲染的 `\\(`、`\\[` 或美元符号公式源码。
9. 在个人空间上传一份测试 PDF，确认重复上传会被拦截，并可以重新解析和删除。
10. 切换到 `demo-b`，确认无法看到 `demo-a` 的个人资料，但仍能访问共享学习小组。
