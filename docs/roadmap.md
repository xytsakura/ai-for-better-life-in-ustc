...
- 确认比赛提交格式和学校资源获取方式。
- 维护者回复等待到 2026-07-27。若届时无明确授权，MVP 自动采用保守降级：仅公开模式、单并发低频访问；搜索缓存 5 至 15 分钟，课程元数据/摘要最长 24 小时；不保存原始 HTML 或全量点评，不处理登录受限附件，不公开登录增强报告。
...
- 独立 `campus.notice.lookup` Agent、标准 Agent Card、10 条固定接入测试和 Gateway 零业务改动证明。
- 课程评价与 RAG 评测夹具模板：来源、许可、数据版本、人工答案、模型/提示版本和评审人。
...
- 只使用允许访问、可展示或模拟脱敏的数据；真实登录增强不作为本周交付。
...
- 公共课程知识库与个人私有知识库隔离。
- 带引用的检索问答 MVP。
- 冻结 40 到 60 条 RAG 评测夹具和恶意文件/提示注入样例。
...
- 资料删除后必须从检索结果中失效。
- 文件先进入隔离区并在无网络、受资源限制的解析环境处理；登录受限附件不进入该链路。
...
- 外部示例 Agent 的接入文档、SDK 薄封装和兼容回归完善；不在本周才首次接入。
- Agent 发现、能力描述、错误处理、SSRF/恶意 Artifact 和降级策略整理。
- 评测脚本和基础指标表。
- 技术文档大纲。
