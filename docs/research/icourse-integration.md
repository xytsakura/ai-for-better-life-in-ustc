...
- 2026-07-19 检查的 `robots.txt` 解释了 search、AI input、AI training 三类 content signal，但未观察到明确的 yes/no 值；这既不是禁止结论，也不构成采集或 AI 处理授权。
...
| <https://icourse.club/robots.txt> | 站点 content signal 说明；未观察到明确许可值 |
...
- 对登录受限附件默认不处理；若用户获得相应授权，本地处理优先，公开报告只展示课程页来源和最小元数据。
...
- “Cookie 不离开设备”不等于内容不离开设备。调用远端模型前必须显示将发送的脱敏片段、模型提供方和用途，并再次取得用户确认；默认不发送登录受限内容。
...
- 附件证据只记录课程页 URL 和页面上下文，不记录真实 `/uploads/files/...` 地址、文件哈希或跨会话可复用引用。
...
- 用户导出公开报告时重新执行公开模式采集。
- Connector 只允许 `https://icourse.club/` origin，默认不跟随跨站跳转，不提交表单、不点赞、不发点评、不改变原站状态。
- 每次读取登录内容前显示页面、字段、附件和输出范围；登录证据标记为 `local_authenticated`，会话结束可一键清除。
- 未取得维护者明确授权时，真实登录增强只供用户本人本地验证，不进入比赛公开演示、服务器事件、共享缓存或团队知识库。
...
| 间接提示注入 | 点评或附件文本诱导模型泄密、越权或调用工具 | 内容只作不可信证据，工具由应用策略允许，增加恶意样例测试 |
| 恶意附件 | PDF/PPTX 主动内容、外链、压缩炸弹或解析器漏洞 | 受限附件默认不入云；授权文件在无网络、受资源限制的沙箱处理 |
...
- 无明确授权时的默认方案：仅使用公开模式；单并发、低频请求；搜索缓存 5 至 15 分钟，课程元数据/摘要最长缓存 24 小时；不保存原始 HTML 或全量点评正文；来源消失、隐藏或投诉时立即删除；不处理登录受限附件；不公开登录增强报告；演示资料使用公开许可、自制或明确授权样例。
...
- 不批量复制或公开再分发全量点评/附件。
- 所有报告结论都要带来源、时间和可见范围。
- 登录受限附件不进入对象存储、内容指纹、向量索引、Artifact、日志或共享缓存。
- 点评、HTML、附件和 OCR 文本只作不可信证据，不得改变工具调用和权限策略。
