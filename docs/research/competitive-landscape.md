...
| Gateway Registry、规则路由和策略 | Build | 第三方 Agent 零业务代码改动接入 |
...
| 评审维度 | 需要展示的证据 |
|---|---|
| 创新性 | 校园 Agent 能力契约、真实外部 Agent 接入、权限和证据随任务跨边界传递 |
| 实用性 | 选课调研和课程复习两条完整用户流程，来源可核验，数据可删除 |
| 技术难度 | A2A/MCP 边界、SSE/轮询恢复、权限预过滤、提示注入防护、固定评测 |
| 完成度 | 一键启动、三 Agent 固定脚本、失败降级、录屏、文档和可复现指标 |
