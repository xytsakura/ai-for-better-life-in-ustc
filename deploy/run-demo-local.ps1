...
$env:HUB_DATABASE_PATH = Join-Path $repoRoot "var\hub\hub.sqlite3"
$env:HUB_PUBLIC_BASE_URL = "http://127.0.0.1:8100"
$env:HUB_MODEL_PROFILES_ENABLED = "true"
$env:HUB_MODEL_PROFILE_MASTER_KEY_FILE = Join-Path $repoRoot "var\hub-secrets\model-profile-master.key"
...
$env:COURSE_AGENT_SECRET_PATH = Join-Path $repoRoot "var\hub-secrets\course-agent.secret"
...
$env:COURSE_AGENT_RUNTIME_DIR = Join-Path $repoRoot "var\course-agent"
...
$env:COURSE_AGENT_HUB_CLIENT_SECRET_FILE = Join-Path $repoRoot "var\hub-secrets\course-agent.secret"
...
if (-not $SkipVerification) {
    & $hubPython (Join-Path $repoRoot "deploy\verify_demo.py") --iterations 1 --minimum-success 1 --output (Join-Path $repoRoot "var\demo-acceptance.json")
