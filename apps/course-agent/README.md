...
2. 在“复习问答”输入“一致连续和连续有什么区别？”，点击“开始检索”，查看模型回答和 `[S1]` 页码引用。
3. 切换到 `队友演示 demo-b`，确认“队友演示的资料”仍为空，再进入共享学习小组确认可以读取共享资料。
4. 在可写空间上传一份 PDF，重复上传会提示重复；删除后再次提问，检索结果不再命中该文件。
...
已验证：`pytest` 4 项通过；双用户个人空间隔离、共享空间访问、连续重解析、重复导入和删除失效均有自动化测试或浏览器回归记录。
